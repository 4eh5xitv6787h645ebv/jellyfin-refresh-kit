...
class ReleasePolicyTests(unittest.TestCase):
    def test_final_is_exact(self) -> None:
        ...
        source = boundary + 10
        result = policy.validate_policy("final", "1.1.0.1", source, source + 1)
        ...
        self.assertEqual(
            result,
            policy.validate_policy("final", "1.1.0.1", source, source + 10_000),
        )
        with self.assertRaises(policy.PolicyError):
            policy.validate_policy("final", "1.0.1.1", source, source + 1)
        with self.assertRaises(policy.PolicyError):
            policy.validate_policy("final", "1.1.0.1", boundary, boundary - 1)
        with self.assertRaises(policy.PolicyError):
            policy.validate_policy("final", "1.1.0.1", boundary - 1, boundary)
...
class ValidationRunTests(unittest.TestCase):
    def test_api_run_and_artifact_identity_are_exact(self) -> None:
        ...
        candidate_ref = "refs/heads/release-candidate/v1.1.0.1"
        run_id = 1234
        ...
        run = {
            ...
            "head_branch": "release-candidate/v1.1.0.1",
...
class RetentionReceiptTests(unittest.TestCase):
    def test_receipt_binds_snapshot_manifest_and_both_packages(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            manifest = root / "manifest.json"
            manifest.write_text("[]\n", encoding="utf-8")
            names = ("jellyfin-refresh-kit_1.1.0.1.zip", "jellyfin-refresh-kit_1.1.0.1_jf12.zip")
            rows = []
            for index, name in enumerate(names):
            ...
            result = retainer.require_receipt_identity(
                receipt, build, manifest, "a" * 40, "1.1.0.1", names
            )
...
class IntegrationEvidenceTests(unittest.TestCase):
    ...
    def make_build(root: pathlib.Path) -> pathlib.Path:
        ...
        for target, stage_name, suffix, framework, abi in (
            ...
            (build / f"jellyfin-refresh-kit_1.1.0.1{suffix}.zip").write_bytes(
    ...
    def self_result(target: str, build: pathlib.Path) -> dict[str, object]:
        ...
        package = build / f"jellyfin-refresh-kit_1.1.0.1{suffix}.zip"
        return {
            "target": target,
            "completed": True,
            "failures": [],
