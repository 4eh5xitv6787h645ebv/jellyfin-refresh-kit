...
`all` starts by cleaning this harness's generated state, then freshly downloads/reinspects all 44 locked archives before it runs the 14 matrices. It writes the exact ordered inspection receipt to `artifacts/all-locked-verification.json` and binds that receipt's SHA-256, row count, byte total, and disposition counts into `artifacts/summary.json`. Each matrix rehashes/reinspects its exact artifact subset, and release validation requires those rows and the result's embedded verification/materialization evidence to match the all-locked receipt. `clean` removes only this directory's `.cache`, `.state`, `artifacts`, and the `rk-compat-*` Compose project's resources.
...
The retained receipt, rather than the roughly 208 MiB of ZIP bytes, is promoted as release evidence. Its 44 rows are validated in lock order against exact dispositions/runtimes, public URLs, archive digests, plugin identity, safe ZIP-derived assembly paths, binary identity tokens, metadata policy, framework evidence where packaged, and complete managed-DLL hash inventories. The 44-archive container-free fetch/inspection and the 19 new package materializations are separate from runtime evidence: the three quarantined and one unsupported archives are inspected but are never described as live runtime passes. Any reported duration for the expanded campaign is taken from a complete coordinated run, not estimated from partial matrices.

The earlier complete nine-matrix diagnostic campaign remains historical evidence only. It pre-dates this 101-row/44-artifact/14-matrix expansion. The first expanded campaign stopped fail-closed in the initial transform matrix when WhisperSubs exposed an upstream exact-key/regex registration conflict; the resulting isolated Whisper matrix is diagnostic evidence only. Promotion requires one coordinated, passing 14-matrix campaign against a single immutable source snapshot; partial and isolated reruns never substitute for it.

Reserve 5 GiB of disk and 3 GiB of RAM for a cold full run. Only one service is started at a time, it has a 3 GiB memory limit and 256 MiB `/tmp`, and successful matrices remove their project volumes while retaining structured host evidence. Report duration from the coordinated complete run rather than extrapolating from partial matrices. The runner resolves `plugin/build` to one verified immutable snapshot before the first matrix and reuses that exact snapshot for the whole run; `RK_COMPAT_BUILD_SNAPSHOT` may select another snapshot under `plugin/.builds` explicitly.
...
- configured opt-in interaction modes (currently Gelato JavaScript injection), required public/authenticated JavaScript Injector content markers (including Media Preview), source-proven exact-cardinality inline functional markers for Custom Tabs, Media Bar, MDBList Ratings, and three PowerToys packages, plus Thumbnail Previews' deferred-script content marker. The 15 probe occurrences also bind the exact selected origin and route, HTTP status, canonical HTTP/1.1 header block, media type, identity coding, framing, strict UTF-8, bounded body size, and marker cardinality; the five PowerToys CSS/JavaScript resources in each install-order variant additionally require source-derived byte lengths and SHA-256 digests;
...
PowerToys' `/_/...` paths encode an MD5 of the assembly full name and embedded-resource name; they are assembly-versioned routes, not content hashes. A same-version rebuild can therefore keep the same URL. Each response-transformer matrix independently fetches all five emitted resources—JellyTag, Privacy Mode, Remote Trailers, and Thumbnail Previews CSS plus Thumbnail Previews JavaScript—and binds their exact locked bytes, SHA-256, MIME essence, and response framing. Remote Trailers' transformed `/web/config.json` is separately required to be a strict JSON object whose `plugins` array contains its exact value once, so a route-shaped or scalar decoy cannot satisfy the functional contract.

Generated or host-owned JavaScript/JSON routes are intentionally semantic rather than byte-pinned: Intro Skipper's transformed Jellyfin bundle, JavaScript Injector's public/private brokers, and the transformed web configuration have narrow per-route size ranges plus exact marker/cardinality contracts. This proves the audited effects in a substantive response but does not claim to parse or fully validate arbitrary JavaScript behavior. Every probe retains its body, raw headers, and status/effective-URL receipt byte-for-byte. The release tree therefore contains exactly 45 content-probe files (15 occurrences times three captures), and independent validation reconstructs every result projection from those raw bytes.
...
Each `artifacts/<matrix>/` directory contains raw server/plugin/diagnostic JSON, before/after generations, shell bodies and headers, compression captures, install metadata, verified network/origin selection in `network.json`, the Jellyfin log, and `result.json`. Matrices with content probes additionally retain `<probe>.txt`, `<probe>.headers`, and `<probe>.status`; direct-writer matrices retain raw `webroot-before.html` and `webroot-after.html`. The collected compatibility tree has an exact 191-file inventory: unexpected, missing, oversized, redirected, malformed, credential-shaped/current-token, or result-divergent probe evidence is rejected. Live authentication tokens are checked in the runner and independently during collection, but are never copied into evidence. Each plugin row in `result.json` carries its artifact, metadata, inventory, load, generation, structured shell attribution/cardinality, order, and outcome evidence. A failed phase still emits a small failure result and retains evidence for inspection; a successful run removes its disposable containers and volumes.
