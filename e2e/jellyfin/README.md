...
- Node.js 22.12 or newer and the repository dependencies installed with `npm ci`
...
generation without reloading the protected document. After blur, the editor
document must converge automatically. This in-place host-upgrade leg is
currently not run by `./test.sh integration` (see the note in `test.sh`); run
`./run.sh host-upgrade` directly to exercise it. The native-uninstall dialog sub-check that this
